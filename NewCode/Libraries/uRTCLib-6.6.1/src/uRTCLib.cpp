...
char* uRTCLib::getTimeStr() {
	refresh();
    static char timeStr[8];
    char timeStrBuff[3];
	memset(timeStr, 0, 8);
    itoa(hour(), timeStrBuff, 10);
    if (hour() < 10) {
      strcpy(timeStr, "0");
      strcat(timeStr, timeStrBuff);
    }
    else
      strcpy(timeStr, timeStrBuff);
    strcat(timeStr, ":");
    memset(timeStrBuff, 0, 3);
    itoa(minute(), timeStrBuff, 10);
    if (minute() < 10)
      strcat(timeStr, "0");
    strcat(timeStr, timeStrBuff);
	return timeStr;
...
char* uRTCLib::getDateStr() {
	refresh();
	static char dateStr[16];
    char dateStrBuff[8];
	memset(dateStr, 0, 16);
    itoa(day(), dateStrBuff, 10);
    if (day() < 10) {
      strcpy(dateStr, "0");
      strcat(dateStr, dateStrBuff);
    }
    else
      strcpy(dateStr, dateStrBuff);
    strcat(dateStr, ".");
    memset(dateStrBuff, 0, 8);
    if (month() < 10)
      strcat(dateStr, "0");
    itoa(month(), dateStrBuff, 10);
    strcat(dateStr, dateStrBuff);
    strcat(dateStr, ".");
    memset(dateStrBuff, 0, 8);
    strcat(dateStr, "20");
    itoa(year(), dateStrBuff, 10);
    strcat(dateStr, dateStrBuff);
	return dateStr;
}

const char* uRTCLib::getMonthStr(bool formatShort) {
	refresh();
	int mon = month();
	if (mon < 0 || mon > 12) {
		Serial.print("Error month is wrong: "); Serial.println(mon);
		return "";
	}
	const char* monthsShort[12] {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
	const char* monthsLong[12] {"Janurary", "Feburary", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
	return (formatShort ? monthsShort[mon - 1] : monthsLong[mon - 1]);
}
