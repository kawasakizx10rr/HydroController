  ...
  if (display::showSystemLogs) {
    if (display::refreshPage) {
      ...
      for (byte i = display::systemLogScrollPos; i < maxLogsToDsiplay; i++) {
        ...
        const char* logPretext = pgm_read_word(&message::notificationsArray[logType]);
        Serial.print(F("Printing log: ")); Serial.print(i); Serial.print(F(", logType: ")); Serial.print(logType); Serial.print(F(", text: ")); Serial.print((const __FlashStringHelper *)logPretext); Serial.print(F(", time: ")); Serial.println(message::timeStrArray[i]);
