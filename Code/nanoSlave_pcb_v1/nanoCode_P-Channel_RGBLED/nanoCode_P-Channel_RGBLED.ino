/*
  ...
  fanTwo          = PORTD 5
  ledRed          = PORTD 7 red
  ledGreen        = PORTB 0 green
  ledBlue         = PORTB 1 blue
  mcp4151_CS      = PORTB 2
*/
...
void setup() {
  ...
  PORTB |= _BV(PB0); // 8 high - off
  PORTB |= _BV(PB1); // 9 high - off

  PORTD &= ~_BV(PD7); // 7 low - on
  delay(500);
  PORTD |= _BV(PD7); // 7 high - off

  PORTB &= ~_BV(PB0); // 8 low - on
  delay(500);
  PORTB |= _BV(PB0); // 8 high - off

  PORTB &= ~_BV(PB1); // 9 low - on
  delay(500);
  PORTB |= _BV(PB1); // 9 high - off
...
void alarm() {
  ...
  if (ledDisabled) { // Turn off the RGB LED
    PORTD |= _BV(PD7); // pin 7 high - off
    PORTB |= _BV(PB0); // pin 8 high - off
    PORTB |= _BV(PB1); // pin 9 high - off
  }
  else if (alarmOption == NO_ALARM) // No alarm set the LED to blue
    PORTB &= ~_BV(PB1); // pins 9 low - on
    ...
    if (alarmOption == MINOR_ALARM) {  // yellow 1 sec on 3 secs off sequence.
      if (count == 1) {
        PORTD &= ~_BV(PD7);// pins 7 low - on
        PORTB &= ~_BV(PB0);// pins 8 low - on
