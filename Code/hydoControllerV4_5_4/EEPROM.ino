void initializeEEPROM() {
  ...
  Serial.print(F("System reset: ")); Serial.println(device::systemReset ? F("true") : F("false"));
...
void saveSystemEEPROM() {
  ...
  EEPROM.put(address, message::systemLogPos);             address += sizeof(message::systemLogPos); 
  //
...
void loadSystemEEPROM() {
  ...
  EEPROM.get(address, message::systemLogPos);             address += sizeof(message::systemLogPos); 
  for (const char& val : user::profileOneName) {
...
void loadUserEEPROM(const byte a_profile) {
  ...
  if (a_profile == 0)
    ...
    loadProfile(1178); // user eeprom size per profile = 531 bytes   493
  else if (a_profile == 2)
    loadProfile(1671);
  else if (a_profile == 3)
    loadProfile(2164);
  else if (a_profile == 4)
    loadProfile(2657); // to 3150 "946 bytes spare of 4096"
  delay(250);
}
