...
void homePage() {
  ...
  if (display::refreshPage) {
    ...
    if (display::homePage == 0) {
      ...
      !user::convertToFarenheit ? tft.print(F("c")) : tft.print(F("f"));
    }
...
void tdsPage() {
  if (display::refreshPage) {
    ...
    tft.print(140, 166, F("Minimum"));
    tft.print(340, 166, F("Maximum"));
    tft.print(550, 166, F("Tolerance"));
  ...
  tft.setFontScale(1);
  // TDS MIN TARGET
  static int targetMinTdsStartPos = 0, targetMinTdsEndPos = 0;
  static int previousTargetMinTds;
  if (display::refreshPage || user::targetMinTds != previousTargetMinTds) {
    tft.fillRect(targetMinTdsStartPos, 238, targetMinTdsEndPos - targetMinTdsStartPos, 50, user::backgroundColor);
    targetMinTdsStartPos = 225 - (tft.getStringWidth(user::targetMinTds) / 2);
    tft.print(targetMinTdsStartPos, 240, user::targetMinTds);
    targetMinTdsEndPos = tft.getFontX();
    previousTargetMinTds = user::targetMinTds;
  }
  // TDS MAX TARGET
  static int targetMaxTdsStartPos = 0, targetMaxTdsEndPos = 0;
  static int previousTargetMaxTds;
  if (display::refreshPage || user::targetMaxTds != previousTargetMaxTds) {
    tft.fillRect(targetMaxTdsStartPos, 238, targetMaxTdsEndPos - targetMaxTdsStartPos, 50, user::backgroundColor);
    targetMaxTdsStartPos = 430 - (tft.getStringWidth(user::targetMaxTds) / 2);
    tft.print(targetMaxTdsStartPos, 240, user::targetMaxTds);
    targetMaxTdsEndPos = tft.getFontX();
    previousTargetMaxTds = user::targetMaxTds;
  }
  // TDS OFFSET
  static int tdsOffsetStartPos = 0, tdsOffsetEndPos = 0;
  static int previousTdsOffset;
  if (display::refreshPage || user::tdsOffset != previousTdsOffset) {
    tft.fillRect(tdsOffsetStartPos, 238, tdsOffsetEndPos - tdsOffsetStartPos, 50, user::backgroundColor);
    tdsOffsetStartPos = 640 - (tft.getStringWidth(user::tdsOffset) / 2);
    tft.print(tdsOffsetStartPos, 240, user::tdsOffset);
    tdsOffsetEndPos = tft.getFontX();
...
void phPage() {
  ...
  tft.setFontScale(1);
  // PH MIN TARGET
  static int targetMinPhStartPos = 0, targetMinPhEndPos = 0;
  static float previousTargetMinPh;
  if (display::refreshPage || user::targetMinPh != previousTargetMinPh) {
    tft.fillRect(targetMinPhStartPos, 238, targetMinPhEndPos - targetMinPhStartPos, 50, user::backgroundColor);
    targetMinPhStartPos = 226 - (tft.getStringWidth(user::targetMinPh, 2) / 2);
    tft.print(targetMinPhStartPos, 240, user::targetMinPh, 2);
    targetMinPhEndPos = tft.getFontX();
    previousTargetMinPh = user::targetMinPh;
  }
  // PH MAX TARGET
  static int targetMaxPhStartPos = 0, targetMaxPhEndPos = 0;
  static float previousTargetMaxPh;
  if (display::refreshPage || user::targetMaxPh != previousTargetMaxPh) {
    tft.fillRect(targetMaxPhStartPos, 238, targetMaxPhEndPos - targetMaxPhStartPos, 50, user::backgroundColor);
    targetMaxPhStartPos = 430 - (tft.getStringWidth(user::targetMaxPh, 2) / 2);
    tft.print(targetMaxPhStartPos, 240, user::targetMaxPh, 2);
    targetMaxPhEndPos = tft.getFontX();
    previousTargetMaxPh = user::targetMaxPh;
  }
  // PH OFFSET
  static int phOffsetStartPos = 0, phOffsetEndPos = 0;
  static float previousPhOffset;
  if (display::refreshPage || user::phOffset != previousPhOffset) {
    tft.fillRect(phOffsetStartPos, 238, phOffsetEndPos - phOffsetStartPos, 50, user::backgroundColor);
    phOffsetStartPos = 640 - (tft.getStringWidth(user::phOffset, 2) / 2);
    tft.print(phOffsetStartPos, 240, user::phOffset, 2);
    phOffsetEndPos = tft.getFontX();
...
void ecPage() {
  ...
  tft.setFontScale(1);
  // EC MIN TARGET
  static int targetMinEcStartPos = 0, targetMinEcEndPos = 0;
  static float previousTargetMinEc;
  if (display::refreshPage || user::targetMinEc != previousTargetMinEc) {
    tft.fillRect(targetMinEcStartPos, 238, targetMinEcEndPos - targetMinEcStartPos, 50, user::backgroundColor);
    targetMinEcStartPos = 225 - (tft.getStringWidth(user::targetMinEc, 2) / 2);
    tft.print(targetMinEcStartPos, 240, user::targetMinEc, 2);
    targetMinEcEndPos = tft.getFontX();
    previousTargetMinEc = user::targetMinEc;
  }
  // EC MAX TARGET
  static int targetMaxEcStartPos = 0, targetMaxEcEndPos = 0;
  static float previousTargetMaxEc;
  if (display::refreshPage || user::targetMaxEc != previousTargetMaxEc) {
    tft.fillRect(targetMaxEcStartPos, 238, targetMaxEcEndPos - targetMaxEcStartPos, 50, user::backgroundColor);
    targetMaxEcStartPos = 430 - (tft.getStringWidth(user::targetMaxEc, 2) / 2);
    tft.print(targetMaxEcStartPos, 240, user::targetMaxEc, 2);
    targetMaxEcEndPos = tft.getFontX();
    previousTargetMaxEc = user::targetMaxEc;
  }
  // EC OFFSET
  static int ecOffsetStartPos = 0, ecOffsetEndPos = 0;
  static float previousEcOffset;
  if (display::refreshPage || user::ecOffset != previousEcOffset) {
    tft.fillRect(ecOffsetStartPos, 238, ecOffsetEndPos - ecOffsetStartPos, 50, user::backgroundColor);
    ecOffsetStartPos = 640 - (tft.getStringWidth(user::ecOffset, 2) / 2);
    tft.print(ecOffsetStartPos, 240, user::ecOffset, 2);
    ecOffsetEndPos = tft.getFontX();
...
void waterPage() {
  ...
  if (display::waterPageScrollPos == 0) {
    ...
    static int targetMinWaterHeightLvlStartPos = 0, targetMinWaterHeightLvlEndPos = 0;
    ...
    if (display::refreshPage || t_targetMinWaterHeight != previoustargetMinWaterHeightLvl) {
      tft.fillRect(targetMinWaterHeightLvlStartPos, 236, targetMinWaterHeightLvlEndPos - targetMinWaterHeightLvlStartPos, 50, user::backgroundColor);
      if (!user::convertToInches) {
        targetMinWaterHeightLvlStartPos = 225 - ((tft.getStringWidth(t_targetMinWaterHeight, 1) / 2) + 8);
        tft.print(targetMinWaterHeightLvlStartPos, 240, t_targetMinWaterHeight, 1);
        tft.setFont(&akashi_36px_Regular);
        tft.setFontScale(1);
        tft.print(tft.getFontX() + 6, 250, F("cm"));
      }
      else {
        targetMinWaterHeightLvlStartPos = 225 - ((tft.getStringWidth(t_targetMinWaterHeight, 1) / 2) + 23);
        tft.print(targetMinWaterHeightLvlStartPos, 240, t_targetMinWaterHeight, 1);
        tft.setFont(&akashi_36px_Regular);
        tft.setFontScale(1);
        tft.print(tft.getFontX() + 6, 230, F("\""));
      }
      targetMinWaterHeightLvlEndPos = tft.getFontX();
      previoustargetMinWaterHeightLvl = t_targetMinWaterHeight;
    }
    // WATER LEVEL MAX TARGET
    tft.setFont(&HallfeticaLargenum_42px_Regular);
    tft.setFontScale(1);
    static int targetMaxWaterHeightLvlStartPos = 0, targetMaxWaterHeightLvlEndPos = 0;
    ...
    if (display::refreshPage || t_targetMaxWaterHeight != previoustargetMaxWaterHeightLvl) {
      tft.fillRect(targetMaxWaterHeightLvlStartPos, 236, targetMaxWaterHeightLvlEndPos - targetMaxWaterHeightLvlStartPos, 50, user::backgroundColor);
      if (!user::convertToInches) {
        targetMaxWaterHeightLvlStartPos = 430 - ((tft.getStringWidth(t_targetMaxWaterHeight, 1) / 2) + 8);
        tft.print(targetMaxWaterHeightLvlStartPos, 240, t_targetMaxWaterHeight, 1);
        tft.setFont(&akashi_36px_Regular);
        tft.setFontScale(1);
        tft.print(tft.getFontX() + 6, 250, F("cm"));
      }
      else {
        targetMaxWaterHeightLvlStartPos = 430 - ((tft.getStringWidth(t_targetMaxWaterHeight, 1) / 2) + 23);
        tft.print(targetMaxWaterHeightLvlStartPos, 240, t_targetMaxWaterHeight, 1);
        tft.setFont(&akashi_36px_Regular);
        tft.setFontScale(1);
        tft.print(tft.getFontX() + 6, 230, F("\""));
      }
      targetMaxWaterHeightLvlEndPos = tft.getFontX();
      previoustargetMaxWaterHeightLvl = t_targetMaxWaterHeight;
    }
    // WATER LEVEL OFFSET
    tft.setFont(&HallfeticaLargenum_42px_Regular);
    tft.setFontScale(1);
    static int waterLvlOffsetStartPos = 0, waterLvlOffsetEndPos = 0;
    ...
    if (display::refreshPage || t_waterHeightOffset != previousWaterLvlOffset) {
      tft.fillRect(waterLvlOffsetStartPos, 236, waterLvlOffsetEndPos - waterLvlOffsetStartPos, 50, user::backgroundColor);
      if (!user::convertToInches) {
        waterLvlOffsetStartPos = 640 - ((tft.getStringWidth(t_waterHeightOffset, 1) / 2) + 8);
        tft.print(waterLvlOffsetStartPos, 240, t_waterHeightOffset, 1);
        tft.setFont(&akashi_36px_Regular);
        tft.setFontScale(1);
        tft.print(tft.getFontX() + 6, 250, F("cm"));
      }
      else {
        waterLvlOffsetStartPos = 640 - ((tft.getStringWidth(t_waterHeightOffset, 1) / 2) + 23);
        tft.print(waterLvlOffsetStartPos, 240, t_waterHeightOffset, 1);
        tft.setFont(&akashi_36px_Regular);
        tft.setFontScale(1);
        tft.print(tft.getFontX() + 6, 230, F("\""));
      }
      waterLvlOffsetEndPos = tft.getFontX();
    ...
    static int targetMinWaterTempStartPos = 0, targetMinWaterTempEndPos = 0;
    ...
    if (display::refreshPage || t_targetMinWaterTemp != previoustargetMinWaterTempWater) {
      tft.fillRect(targetMinWaterTempStartPos, 236, targetMinWaterTempEndPos - targetMinWaterTempStartPos, 50, user::backgroundColor);
      targetMinWaterTempStartPos = 225 - ((tft.getStringWidth(t_targetMinWaterTemp, 1) / 2) + 8);
      tft.print(targetMinWaterTempStartPos, 240, t_targetMinWaterTemp, 1);
      tft.setFont(&akashi_36px_Regular);
      tft.setFontScale(1);
      tft.setCursor(tft.getFontX() + 6, 230);
      !user::convertToFarenheit ? tft.print(F("c")) : tft.print(F("f"));
      targetMinWaterTempEndPos = tft.getFontX();
      previoustargetMinWaterTempWater = t_targetMinWaterTemp;
    }
    // WATER TEMPERATURE MAX TARGET
    tft.setFont(&HallfeticaLargenum_42px_Regular);
    tft.setFontScale(1);
    static int targetMaxWaterTempStartPos = 0, targetMaxWaterTempEndPos = 0;
    ...
    if (display::refreshPage || t_targetMaxWaterTemp != previoustargetMaxWaterTempWater) {
      tft.fillRect(targetMaxWaterTempStartPos, 236, targetMaxWaterTempEndPos - targetMaxWaterTempStartPos, 50, user::backgroundColor);
      targetMaxWaterTempStartPos = 430 - ((tft.getStringWidth(t_targetMaxWaterTemp, 1) / 2) + 8);
      tft.print(targetMaxWaterTempStartPos, 240, t_targetMaxWaterTemp, 1);
      tft.setFont(&akashi_36px_Regular);
      tft.setFontScale(1);
      tft.setCursor(tft.getFontX() + 6, 230);
      !user::convertToFarenheit ? tft.print(F("c")) : tft.print(F("f"));
      targetMaxWaterTempEndPos = tft.getFontX();
      previoustargetMaxWaterTempWater = t_targetMaxWaterTemp;
    }
    // WATER TEMPERATURE OFFSET
    tft.setFont(&HallfeticaLargenum_42px_Regular);
    tft.setFontScale(1);
    static int waterTempOffsetStartPos = 0, waterTempOffsetEndPos = 0;
    ...
    if (display::refreshPage || t_waterTempOffset != previouswaterTempOffset) {
      tft.fillRect(waterTempOffsetStartPos, 236, waterTempOffsetEndPos - waterTempOffsetStartPos, 50, user::backgroundColor);
      waterTempOffsetStartPos = 640 - ((tft.getStringWidth(t_waterTempOffset, 1) / 2) + 8);
      tft.print(waterTempOffsetStartPos, 240, t_waterTempOffset, 1);
      tft.setFont(&akashi_36px_Regular);
      tft.setFontScale(1);
      tft.setCursor(tft.getFontX() + 6, 230);
      !user::convertToFarenheit ? tft.print(F("c")) : tft.print(F("f"));
      waterTempOffsetEndPos = tft.getFontX();
...
void fansPage() {
  ...
  if (display::fansPage == 0) {
    ...
    static int targetMinAirTempStartPos = 0, targetMinAirTempEndPos = 0;
    ...
    if (display::refreshPage || t_targetMinAirTemp != previoustargetMinAirTempAir) {
      tft.fillRect(targetMinAirTempStartPos, 236, targetMinAirTempEndPos - targetMinAirTempStartPos, 50, user::backgroundColor);
      targetMinAirTempStartPos = 225 - ((tft.getStringWidth(t_targetMinAirTemp, 1) / 2) + 8);
      tft.print(targetMinAirTempStartPos, 240, t_targetMinAirTemp, 1);
      tft.setFont(&akashi_36px_Regular);
      tft.setFontScale(1);
      tft.setCursor(tft.getFontX() + 6, 230);
      !user::convertToFarenheit ? tft.print(F("c")) : tft.print(F("f"));
      targetMinAirTempEndPos = tft.getFontX();
      previoustargetMinAirTempAir = t_targetMinAirTemp;
    }
    // AIR TEMPERATURE MAX TARGET
    tft.setFont(&HallfeticaLargenum_42px_Regular);
    tft.setFontScale(1);
    static int targetMaxAirTempStartPos = 0, targetMaxAirTempEndPos = 0;
    ...
    if (display::refreshPage || t_targetMaxAirTemp != previoustargetMaxAirTempAir) {
      tft.fillRect(targetMaxAirTempStartPos, 236, targetMaxAirTempEndPos - targetMaxAirTempStartPos, 50, user::backgroundColor);
      targetMaxAirTempStartPos = 430 - ((tft.getStringWidth(t_targetMaxAirTemp, 1) / 2) + 8);
      tft.print(targetMaxAirTempStartPos, 240, t_targetMaxAirTemp, 1);
      tft.setFont(&akashi_36px_Regular);
      tft.setFontScale(1);
      tft.setCursor(tft.getFontX() + 6, 230);
      !user::convertToFarenheit ? tft.print(F("c")) : tft.print(F("f"));
      targetMaxAirTempEndPos = tft.getFontX();
      previoustargetMaxAirTempAir = t_targetMaxAirTemp;
    }
    // AIR TEMPERATURE OFFSET
    tft.setFont(&HallfeticaLargenum_42px_Regular);
    tft.setFontScale(1);
    static int airTempOffsetStartPos = 0, airTempOffsetEndPos = 0;
    ...
    if (display::refreshPage || t_airTempOffset != previousAirTempOffset) {
      tft.fillRect(airTempOffsetStartPos, 236, airTempOffsetEndPos - airTempOffsetStartPos, 50, user::backgroundColor);
      airTempOffsetStartPos = 640 - ((tft.getStringWidth(t_airTempOffset, 1) / 2) + 8);
      tft.print(airTempOffsetStartPos, 240, t_airTempOffset, 1);
      tft.setFont(&akashi_36px_Regular);
      tft.setFontScale(1);
      tft.setCursor(tft.getFontX() + 6, 230);
      !user::convertToFarenheit ? tft.print(F("c")) : tft.print(F("f"));
      airTempOffsetEndPos = tft.getFontX();
    ...
    tft.setFontScale(1);
    // HUMIDITY MIN TARGET
    static int targetMinHumidityStartPos = 0, targetMinHumidityEndPos = 0;
    static float previousTargetMinHumidity;
    if (display::refreshPage || user::targetMinHumidity != previousTargetMinHumidity) {
      tft.fillRect(targetMinHumidityStartPos, 238, targetMinHumidityEndPos - targetMinHumidityStartPos, 50, user::backgroundColor);
      targetMinHumidityStartPos = 225 - ((tft.getStringWidth(user::targetMinHumidity, 1) / 2) + 8);
      tft.print(targetMinHumidityStartPos, 240, user::targetMinHumidity, 1);
      tft.setFont(&akashi_36px_Regular);
      tft.setFontScale(1);
      tft.print(tft.getFontX() + 6, 249, F("%"));
      targetMinHumidityEndPos = tft.getFontX();
      previousTargetMinHumidity = user::targetMinHumidity;
    }
    // HUMIDITY MAX TARGET
    tft.setFont(&HallfeticaLargenum_42px_Regular);
    tft.setFontScale(1);
    static int targetMaxHumidityStartPos = 0, targetMaxHumidityEndPos = 0;
    static float previousTargetMaxHumidity;
    if (display::refreshPage || user::targetMaxHumidity != previousTargetMaxHumidity) {
      tft.fillRect(targetMaxHumidityStartPos, 238, targetMaxHumidityEndPos - targetMaxHumidityStartPos, 50, user::backgroundColor);
      targetMaxHumidityStartPos = 430 - ((tft.getStringWidth(user::targetMaxHumidity, 1) / 2) + 8);
      tft.print(targetMaxHumidityStartPos, 240, user::targetMaxHumidity, 1);
      tft.setFont(&akashi_36px_Regular);
      tft.setFontScale(1);
      tft.print(tft.getFontX() + 6, 249, F("%"));
      targetMaxHumidityEndPos = tft.getFontX();
      previousTargetMaxHumidity = user::targetMaxHumidity;
    }
    // HUMIDITY OFFSET
    tft.setFont(&HallfeticaLargenum_42px_Regular);
    tft.setFontScale(1);
    static int humidityOffsetStartPos = 0, humidityOffsetEndPos = 0;
    static float previousHumidityOffset;
    if (display::refreshPage || user::humidityOffset != previousHumidityOffset) {
      tft.fillRect(humidityOffsetStartPos, 238, humidityOffsetEndPos - humidityOffsetStartPos, 50, user::backgroundColor);
      humidityOffsetStartPos = 640 - ((tft.getStringWidth(user::humidityOffset, 1) / 2) + 8);
      tft.print(humidityOffsetStartPos, 240, user::humidityOffset, 1);
      tft.setFont(&akashi_36px_Regular);
      tft.setFontScale(1);
      tft.print(tft.getFontX() + 6, 249, F("%"));
      humidityOffsetEndPos = tft.getFontX();
  ...
  if (display::showSystemLogs) {
    if (display::refreshPage) {
      ...
      for (byte i = display::systemLogScrollPos; i < maxLogsToDsiplay; i++) {
        const char* logPretext = PROGMEM_read(&message::notificationsArray[i]);
    ...
    if (display::settingsPageOneScrollPos <= 2) {
      ...
      if (display::refreshPage || previousNumberOfDosers != user::numberOfDosers) {
        ...
        tft.fillRect(508, 258 - scrollMargin, previousNumberOfDosersPosition - 506, 32, user::backgroundColor);
    ...
    if (display::settingsPageOneScrollPos <= 3) {
      ...
      if (display::refreshPage || previousDosingInterval != user::dosingInterval) {
        ...
        tft.fillRect(508, 302 - scrollMargin, previousDosingIntervalPosition - 506, 32, user::backgroundColor);
