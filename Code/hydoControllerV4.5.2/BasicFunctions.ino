...
float adjustValue(float a_val, float a_increment, const float& a_min, const float& a_max) {
  ...
  if (display::currentTouch - display::lastTouch > 10000UL)
    a_increment *= 10.0;
  if (a_increment < 0 && a_val > a_min + abs(a_increment))
    a_val += a_increment;
  else if (a_increment < 0)
    a_val = a_min;
  if (a_increment > 0 && a_val < a_max - a_increment)
    a_val += a_increment;
  else if (a_increment > 0)
    a_val = a_max;
  //Serial.print(F("Adjust value = ")); Serial.println(val);
  return a_val;
}
...
void saveLogMessage(const byte& a_logType) {
  ...
  Serial.print(F("Added new log: "));
  const char* logPretext = PROGMEM_read(&message::notificationsArray[a_logType]);
  Serial.println((const __FlashStringHelper *)logPretext);
