...
int sliderXTouch() {
  ...
  if (display::currentTouch > display::lastTouch) {
    if (display::touch_x > display::lastRightTouch) {
      scrollRightCount++;
      scrollLeftCount = 0;
      display::lastRightTouch = display::touch_x;
    }
    else if (display::touch_x < display::lastLeftTouch) {
      scrollLeftCount++;
      scrollRightCount = 0;
      display::lastLeftTouch = display::touch_x;
    }
  }
  if (scrollRightCount >= 30) {
    returnVal = 1;
    scrollRightCount = 0;
    display::lastRightTouch = 0;
  }
  else if (scrollLeftCount >= 30) {
    returnVal = -1;
    scrollLeftCount = 0;
    display::lastLeftTouch = tft.width();
  }
...
int sliderYTouch() {
  ...
  int returnVal = 0;
  if (display::currentTouch > display::lastTouch) {
    if (display::touch_y > display::lastDownTouch) {
      scrollDownCount++;
      scrollUpCount = 0;
      display::lastDownTouch = display::touch_y;
    }
    else if (display::touch_y < display::lastUpTouch) {
      scrollUpCount++;
      scrollDownCount = 0;
      display::lastUpTouch = display::touch_y;
    }
  }
  if (scrollDownCount >= 30) {
    returnVal = 1;
    scrollDownCount = 0;
    display::lastDownTouch = 0;
  }
  else if (scrollUpCount >= 30) {
    returnVal = -1;
    scrollUpCount = 0;
    display::lastUpTouch = tft.height();
  }
...
void graphsPageTouched() {
  if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 90 && display::touch_y <= 124)
    setGraphOption(1);
  else if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 124 && display::touch_y <= 158)
    setGraphOption(2);
  else if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 158 && display::touch_y <= 192)
    setGraphOption(3);
  else if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 192 && display::touch_y <= 230)
    setGraphOption(4);
  else if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 230 && display::touch_y <= 268)
    setGraphOption(5);
  else if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 268 && display::touch_y <= 310)
    setGraphOption(6);
  else if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 310 && display::touch_y <= 354)
    setGraphOption(7);
  else if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 354 && display::touch_y <= 394)
    setGraphOption(8);
  else if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 394 && display::touch_y <= 434)
    setGraphOption(9);
  else if (display::touch_x >= 760 && display::touch_x <= 835 && display::touch_y >= 434 && display::touch_y <= 474)
    setGraphOption(10);
}
  ...
  if (user::setRTCtime) {
    ...
    if (display::settingsPageZeroScrollPos <= 4) {
      if (touchX >= 700 && touchX <= 770 && touchY >= 346 - scrollMargin && touchY <= 376 - scrollMargin) { // convertToFarenheit
        user::convertToFarenheit  = !user::convertToFarenheit;
    ...
    if (display::settingsPageZeroScrollPos >= 1) {
      if (touchX >= 700 && touchX <= 770 && touchY >= 434 - scrollMargin && touchY <= 464 - scrollMargin) { // convertToTds
        ...
        device::settingsAdjusted = true;
        user::convertToTds ? drawPageIconQuick(5, 15, 10, ppmIcon) : drawPageIconQuick(5, 15, 10, ecIcon);
      }
    }
    if (display::settingsPageZeroScrollPos >= 2) {
...
void settingsOnePageTouched() {
  if (display::showSystemLogs) {
    if (display::touch_x >= 710 && display::touch_x <= 799 && display::touch_y >= 110 && display::touch_y <= 160) { // exit
      beep();
      display::showSystemLogs = false;
      display::refreshPage = true;
      clearPage();
    ...
    if (display::settingsPageOneScrollPos <= 2) {
      if (touchX >= 700 && touchX <= 715 && touchY >= 258 - scrollMargin && touchY <= 288 - scrollMargin) { // numberOfDosers down
        user::numberOfDosers = adjustValue(user::dosingInterval, -1, 4, 6);
        device::settingsAdjusted = true;
      }
...
void ecTdsPageTouched() {
  if (user::convertToTds) {
    if (display::touch_x >= 145 && display::touch_x <= 210 && display::touch_y >= 340 && display::touch_y <= 420) {  // min target ppm down
      user::targetMinTds = adjustValue(user::targetMinTds, -1, 1, user::targetMaxTds - 1);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 225 && display::touch_x <= 290 && display::touch_y >= 340 && display::touch_y <= 420) {  // min target ppm up
      user::targetMinTds = adjustValue(user::targetMinTds, 1, 1, user::targetMaxTds - 1);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 350 && display::touch_x <= 415 && display::touch_y >= 340 && display::touch_y <= 420) {  // max target ppm down
      user::targetMaxTds = adjustValue(user::targetMaxTds, -1, user::targetMinTds + 1, 999);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 430 && display::touch_x <= 495 && display::touch_y >= 340 && display::touch_y <= 420) {  // max target ppm up
      user::targetMaxTds = adjustValue(user::targetMaxTds, 1, user::targetMinTds + 1, 999);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 560 && display::touch_x <= 625 && display::touch_y >= 340 && display::touch_y <= 420) {  // ppm tolerance down
      user::tdsOffset = adjustValue(user::tdsOffset, -1, 0, 50);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 640 && display::touch_x <= 705 && display::touch_y >= 340 && display::touch_y <= 420) {  // ppm tolerance up
      user::tdsOffset = adjustValue(user::tdsOffset, 1, 0, 50);
      device::ecValsAdjusted = true;
    }
  }
  else {
    if (display::touch_x >= 145 && display::touch_x <= 210 && display::touch_y >= 340 && display::touch_y <= 420) {  // min target ec down
      user::targetMinEc = adjustValue(user::targetMinEc, -0.01, 0.1, user::targetMaxEc - 0.1);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 225 && display::touch_x <= 290 && display::touch_y >= 340 && display::touch_y <= 420) {  // min target ec up
      user::targetMinEc = adjustValue(user::targetMinEc, 0.01, 0.1, user::targetMaxEc - 0.1);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 350 && display::touch_x <= 415 && display::touch_y >= 340 && display::touch_y <= 420) {  // max target ec down
      user::targetMaxEc = adjustValue(user::targetMaxEc, -0.01, user::targetMinEc + 0.1, 6);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 430 && display::touch_x <= 495 && display::touch_y >= 340 && display::touch_y <= 420) {  // max target ec up
      user::targetMaxEc = adjustValue(user::targetMaxEc, 0.01, user::targetMinEc + 0.1, 6);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 560 && display::touch_x <= 625 && display::touch_y >= 340 && display::touch_y <= 420) {  // ec tolerance down
      user::ecOffset = adjustValue(user::ecOffset, -0.01, 0, 1);
      device::ecValsAdjusted = true;
    }
    else if (display::touch_x >= 640 && display::touch_x <= 705 && display::touch_y >= 340 && display::touch_y <= 420) {  // ec tolerance up
      user::ecOffset = adjustValue(user::ecOffset, 0.01, 0, 1);
      device::ecValsAdjusted = true;
    }
  }
...
void phPageTouched() {
  if (display::touch_x >= 145 && display::touch_x <= 210 && display::touch_y >= 340 && display::touch_y <= 420) { // min target ph down
    user::targetMinPh = adjustValue(user::targetMinPh, -0.01, 1, user::targetMaxPh - 0.1);
    device::phValsAdjusted = true;
  }
  else if (display::touch_x >= 225 && display::touch_x <= 290 && display::touch_y >= 340 && display::touch_y <= 420) { // min target ph up
    user::targetMinPh = adjustValue(user::targetMinPh, 0.01, 1,  user::targetMaxPh - 0.1);
    device::phValsAdjusted = true;
  }
  else if (display::touch_x >= 350 && display::touch_x <= 415 && display::touch_y >= 340 && display::touch_y <= 420) { // max target ph down
    user::targetMaxPh = adjustValue(user::targetMaxPh, -0.01, user::targetMinPh + 0.1, 14);
    device::phValsAdjusted = true;
  }
  else if (display::touch_x >= 430 && display::touch_x <= 495 && display::touch_y >= 340 && display::touch_y <= 420) { // max target ph up
    user::targetMaxPh = adjustValue(user::targetMaxPh, 0.01, user::targetMinPh + 0.1, 14);
    device::phValsAdjusted = true;
  }
  else if (display::touch_x >= 560 && display::touch_x <= 625 && display::touch_y >= 340 && display::touch_y <= 420) { // ph tolerance down
    user::phOffset = adjustValue(user::phOffset, -0.01, 0.0, 1.0);
    device::phValsAdjusted = true;
  }
  else if (display::touch_x >= 640 && display::touch_x <= 705 && display::touch_y >= 340 && display::touch_y <= 420) { // ph tolerance up
    user::phOffset = adjustValue(user::phOffset, 0.01, 0.0, 1.0);
    device::phValsAdjusted = true;
  }
}
...
void waterPageTouched() {
  if (display::waterPageScrollPos == 0) {
    if (display::touch_x >= 145 && display::touch_x <= 210 && display::touch_y >= 340 && display::touch_y <= 420) { // min target water level down
      if (user::convertToInches)
    ...
    else if (display::touch_x >= 225 && display::touch_x <= 290 && display::touch_y >= 340 && display::touch_y <= 420) { // min target water level up
      if (user::convertToInches)
        user::targetMinWaterHeightInches = adjustValue(user::targetMinWaterHeightInches, 0.1, 1, user::targetMaxWaterHeightInches - 1);
      else
        user::targetMinWaterHeight = adjustValue(user::targetMinWaterHeight, 0.1, 1, user::targetMaxWaterHeight - 1);
    ...
    else if (display::touch_x >= 350 && display::touch_x <= 415 && display::touch_y >= 340 && display::touch_y <= 420) { // max target water level down
      if (user::convertToInches)
    ...
    else if (display::touch_x >= 430 && display::touch_x <= 495 && display::touch_y >= 340 && display::touch_y <= 420) { // max target water level up
      if (user::convertToInches)
        user::targetMaxWaterHeightInches = adjustValue(user::targetMaxWaterHeightInches, 0.1, user::targetMinWaterHeightInches + 1, 99);
      else
        user::targetMaxWaterHeight = adjustValue(user::targetMaxWaterHeight, 0.1, user::targetMinWaterHeight + 1, 99);
    ...
    else if (display::touch_x >= 560 && display::touch_x <= 625 && display::touch_y >= 340 && display::touch_y <= 420) { // water level tolerance down
      if (user::convertToInches)
        user::waterHeightOffsetInches = adjustValue(user::waterHeightOffsetInches, -0.1, 0, 10);
      else
        user::waterHeightOffset = adjustValue(user::waterHeightOffset, -0.1, 0, 10);
      device::waterValsAdjusted = true;
    }
    else if (display::touch_x >= 640 && display::touch_x <= 705 && display::touch_y >= 340 && display::touch_y <= 420) { // water level tolerance up
      if (user::convertToInches)
        user::waterHeightOffsetInches = adjustValue(user::waterHeightOffsetInches, 0.1, 0, 10);
      else
        user::waterHeightOffset = adjustValue(user::waterHeightOffset, 0.1, 0, 10);
    ...
    if (display::touch_x >= 145 && display::touch_x <= 210 && display::touch_y >= 340 && display::touch_y <= 420) { // min target temperature down
    ...
    else if (display::touch_x >= 225 && display::touch_x <= 290 && display::touch_y >= 340 && display::touch_y <= 420) { // min target temperature up
      if (user::convertToFarenheit)
        user::targetMinWaterTempF = adjustValue(user::targetMinWaterTempF, 0.1, 1, user::targetMaxWaterTempF - 0.1);
      else
        user::targetMinWaterTemp = adjustValue(user::targetMinWaterTemp, 0.1, 1, user::targetMaxWaterTemp - 0.1);
    ...
    else if (display::touch_x >= 350 && display::touch_x <= 415 && display::touch_y >= 340 && display::touch_y <= 420) { // max target temperature down
      if (user::convertToFarenheit)
    ...
    else if (display::touch_x >= 430 && display::touch_x <= 495 && display::touch_y >= 340 && display::touch_y <= 420) { // max target temperature up
      if (user::convertToFarenheit)
    ...
    else if (display::touch_x >= 560 && display::touch_x <= 625 && display::touch_y >= 340 && display::touch_y <= 420) { // temperature tolerance down
      if (user::convertToFarenheit)
        user::waterTempOffsetF = adjustValue(user::waterTempOffsetF, -0.1, 0, 1);
      else
        user::waterTempOffset = adjustValue(user::waterTempOffset, -0.1, 0, 1);
      device::waterValsAdjusted = true;
    }
    else if (display::touch_x >= 640 && display::touch_x <= 705 && display::touch_y >= 340 && display::touch_y <= 420) { // temperature tolerance up
      if (user::convertToFarenheit)
        user::waterTempOffsetF = adjustValue(user::waterTempOffsetF, 0.1, 0, 1);
      else
        user::waterTempOffset = adjustValue(user::waterTempOffset, 0.1, 0, 1);
...
void fansPageTouched() {
  if (display::fansPage == 0) {
    ...
    if (display::touch_x >= 145 && display::touch_x <= 210 && display::touch_y >= 340 && display::touch_y <= 420) { // min target air temp down
      if (user::convertToFarenheit)
    ...
    else if (display::touch_x >= 225 && display::touch_x <= 290 && display::touch_y >= 340 && display::touch_y <= 420) { // min target air temp up
      if (user::convertToFarenheit)
    ...
    else if (display::touch_x >= 350 && display::touch_x <= 415 && display::touch_y >= 340 && display::touch_y <= 420) { // max target air temp down
      if (user::convertToFarenheit)
    ...
    else if (display::touch_x >= 430 && display::touch_x <= 495 && display::touch_y >= 340 && display::touch_y <= 420) { // max target air temp up
      if (user::convertToFarenheit)
    ...
    else if (display::touch_x >= 560 && display::touch_x <= 625 && display::touch_y >= 340 && display::touch_y <= 420) { // air temp tolerance down
      if (user::convertToFarenheit)
        user::airTempOffsetF = adjustValue(user::airTempOffsetF, -0.1, 0, 10);
      else
        user::airTempOffset = adjustValue(user::airTempOffset, -0.1, 0, 10);
      device::fanValsAdjusted = true;
    }
    else if (display::touch_x >= 640 && display::touch_x <= 705 && display::touch_y >= 340 && display::touch_y <= 420) { // air temp tolerance up
      if (user::convertToFarenheit)
        user::airTempOffsetF = adjustValue(user::airTempOffsetF, 0.1, 0, 10);
      else
        user::airTempOffset = adjustValue(user::airTempOffset, 0.1, 0, 10);
    ...
    if (display::touch_x >= 145 && display::touch_x <= 210 && display::touch_y >= 340 && display::touch_y <= 420) { // min target humidity down
      user::targetMinHumidity = adjustValue(user::targetMinHumidity, -0.1, 1, 100);
      device::fanValsAdjusted = true;
    }
    else if (display::touch_x >= 225 && display::touch_x <= 290 && display::touch_y >= 340 && display::touch_y <= 420) { // min target humidity up
      user::targetMinHumidity = adjustValue(user::targetMinHumidity, 0.1, 1, user::targetMaxHumidity - 1);
      device::fanValsAdjusted = true;
    }
    else if (display::touch_x >= 350 && display::touch_x <= 415 && display::touch_y >= 340 && display::touch_y <= 420) { // max target humidity down
      user::targetMaxHumidity = adjustValue(user::targetMaxHumidity, -0.1, user::targetMinHumidity + 1, 100);
      device::fanValsAdjusted = true;
    }
    else if (display::touch_x >= 430 && display::touch_x <= 495 && display::touch_y >= 340 && display::touch_y <= 420) { // max target humidity up
      user::targetMaxHumidity = adjustValue(user::targetMaxHumidity, 0.1, 1, 100);
      device::fanValsAdjusted = true;
    }
    else if (display::touch_x >= 560 && display::touch_x <= 625 && display::touch_y >= 340 && display::touch_y <= 420) { // humidity tolerance down
      user::humidityOffset = adjustValue(user::humidityOffset, -0.1, 0, 10);
      device::fanValsAdjusted = true;
    }
    else if (display::touch_x >= 640 && display::touch_x <= 705 && display::touch_y >= 340 && display::touch_y <= 420) { // humidity tolerance up
      user::humidityOffset = adjustValue(user::humidityOffset, 0.1, 0, 10);
      device::fanValsAdjusted = true;
    }
  }
