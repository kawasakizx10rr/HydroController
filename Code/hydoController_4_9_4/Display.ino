...
void homePage() {
  ...
  //if (display::refreshPage || device::newGraphData) {
    if (display::homePage == 0) {
      ...
      if (device::lightOn) {
        if (user::convertToF)
          drawSensorSlide(convertToF(sensor::airTemp), user::targetDayMinAirTempF, user::targetDayMaxAirTempF, sensor::airTemperatureArrayF, true, 1);
        else
          drawSensorSlide(sensor::airTemp, user::targetDayMinAirTemp, user::targetDayMaxAirTemp, sensor::airTemperatureArray, true, 1);
      }
      else {
         if (user::convertToF)
          drawSensorSlide(convertToF(sensor::airTemp), user::targetNightMinAirTempF, user::targetNightMaxAirTempF, sensor::airTemperatureArrayF, true, 1);
        else
          drawSensorSlide(sensor::airTemp, user::targetNightMinAirTemp, user::targetNightMaxAirTemp, sensor::airTemperatureArray, true, 1);       
      }
    }
    else if (display::homePage == 9) {
      if (device::lightOn)
        drawSensorSlide(sensor::humidity, user::targetDayMinHumidity, user::targetDayMaxHumidity, sensor::humidityArray, true, 1);
      else
        drawSensorSlide(sensor::humidity, user::targetNightMinHumidity, user::targetNightMaxHumidity, sensor::humidityArray, true, 1);
    }
  //}
}
// =============================================================================================================================================================================================
...
void fansPage() {
  if (display::refreshPage)
    drawSlideIcons(110, 112, display::fansPage, 7);
  if (display::fansPage == 0) {
    ...
    if (display::refreshPage) {
      ...
      tft.print(324, 124, F("Fan settings"));
      tft.setTextColor(RA8875_BLACK, user::backgroundColor);
