...
// Draw a menu icon
void drawPageIcon(const byte& a_page, const short& a_x, const short a_y, const byte* a_icon, const int& a_arraySize, const byte& a_width, const byte& a_height) {
  if (display::page == a_page) {
    ...
    tft.drawXbmpArray(a_icon, a_arraySize, a_x, a_y, a_width, a_height, display::RA8875_GREY, 1);
  }
}

void drawPageIconQuick(const byte& a_page, const short& a_x, const short a_y, const byte* a_icon, const int& a_arraySize, const byte& a_width, const byte& a_height) {
  if (display::page == a_page) {
    tft.drawXbmpArray(a_icon, a_arraySize, a_x, a_y, a_width, a_height, RA8875_BLUE, 1);
  }
  else {
...
void drawGraphIcons() {
  drawMenuIcon(display::graphOption, 1, 747, 98, miniPmIcon, 115, 40, 23);
  drawMenuIcon(display::graphOption, 2, 747, 131, miniPhIcon, 112, 39, 23);
  drawMenuIcon(display::graphOption, 3, 748, 166, miniEcIcon, 109, 38, 23);
  drawMenuIcon(display::graphOption, 4, 746, 199, miniCo2Icon, 130, 40, 26);
  drawMenuIcon(display::graphOption, 5, 753, 234, miniWaterTempIcon, 108, 27, 32);
  drawMenuIcon(display::graphOption, 6, 754, 273, miniWaterLevelIcon, 101, 27, 30);
  drawMenuIcon(display::graphOption, 7, 749, 312, miniFanOneIcon, 157, 35, 36);
  drawMenuIcon(display::graphOption, 8, 749, 356, miniFanTwoIcon, 157, 35, 36);
  drawMenuIcon(display::graphOption, 9, 754, 396, miniAirTempIcon, 121, 27, 36);
  drawMenuIcon(display::graphOption, 10, 750, 438, miniHumidityIcon, 136, 34, 32);
  tft.drawRect(742, 88, 48, 391, RA8875_BLACK);
  tft.drawRect(743, 89, 46, 389, RA8875_BLACK);
}

// draws a mini graph icon
void drawMenuIcon(const byte& a_currentPage, const byte& a_newPage, const short& a_x, const short a_y, const byte* a_icon, const int& a_arraySize, const byte& a_width, const byte& a_height) {
...
void drawSensorSlide(const float & a_sensorData, const float & a_minTarget, const float & a_maxTarget, const float * a_arrayData, const byte & a_percision) {
